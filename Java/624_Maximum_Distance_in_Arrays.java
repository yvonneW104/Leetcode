...
class Solution {
    public int maxDistance(List<List<Integer>> arrays) {
        ...
        for (int i = 1; i < arrays.size(); i++) {
            int sub_min = arrays.get(i).get(0);
            int sub_max = arrays.get(i).get(arrays.get(i).size() - 1);
            res = Math.max(res, Math.abs(sub_min - max));
            res = Math.max(res, Math.abs(sub_max - min));
            min = Math.min(min, sub_min);
