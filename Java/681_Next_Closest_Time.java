...
assumed that the returned time is next day's time since it is smaller than the input 
time numerically.
...
class Solution {
    public String nextClosestTime(String time) {
        int cur_time = 60 * Integer.parseInt(time.substring(0, 2));
        cur_time += Integer.parseInt(time.substring(3));
        
        Set<Integer> valid_digit = new HashSet<Integer>();
        for (char c : time.toCharArray()) 
            if (c != ':')   valid_digit.add(c - '0');
        
        while (true) {
            cur_time = (cur_time + 1) % (24 * 60);
            int[] digits = new int[]{cur_time / 60 / 10, cur_time / 60 % 10, cur_time % 60 / 10, cur_time % 60 % 10};
            search: {
                for (int digit : digits) 
                    if (!valid_digit.contains(digit)) 
                        break search;
                return String.format("%02d:%02d", cur_time / 60, cur_time % 60);
            }
        }
    }
}