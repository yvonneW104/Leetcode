...
Of the subarrays that have the same degree: [1, 2, 2, 3, 1], [1, 2, 2, 3], [2, 2, 3, 1], 
    ...
    public int findShortestSubArray(int[] nums) {
        ...
        for (int i = 0; i < nums.length; i++) {
            ...
            if (!map.containsKey(nums[i])) {
                ...
                temp_array = map.get(nums[i]);
            }
            temp_array.add(i);
            map.put(nums[i], temp_array);
            
            int size = temp_array.size();
            if (size > max_freq) {
                res = temp_array.get(size - 1) - temp_array.get(0) + 1;
                max_freq = size;
            } else if (size == max_freq) {
                res = Math.min(res, temp_array.get(size - 1) - temp_array.get(0) + 1);
            }
        }
        return res;
    }
