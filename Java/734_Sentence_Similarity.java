...
Given two sentences words1, words2 (each represented as an array 
of strings), and a list of similar word pairs pairs, determine if 
two sentences are similar.

For example, "great acting skills" and "fine drama talent" are 
...
Also, a word is always similar with itself. For example, the 
sentences words1 = ["great"], words2 = ["great"], pairs = [] are 
similar, even though there are no specified similar word pairs.
...
class Solution {
	public boolean areSentencesSimilar(String[] words1, String[] words2, String[][] pairs) {
		...
		Set<String> pairest = new HashSet<String>();
		for (String[] pair: pairs)
			pairest.add(pair[0] + "#" + pair[1]);

		for (int = 0; i < words1.length; i++) {
			if (!words1[i].equals(words2[i]) &&
				!pairest.contains(words1[i] + "#" + words2[i]) ||
				!pairest.contains(words2[i] + "#" + words1[i]))
				return false;
		}
		return true;
	}
