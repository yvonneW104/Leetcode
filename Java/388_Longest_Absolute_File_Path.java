...
class Solution {
	public int lengthLongesPath (String input) {
		Stack<Integer> stack = new Stack<Integer>();
		stack.push(0);
		int maxlen = 0;

		for (String word : input.split("\n")) {
			int curlev = word.lastIndexOf('\t') + 1;
			int curlen = word.length() - curlev;

			while (stack.size() > curlev + 1) 
				stack.pop();

			if (word.contains(".")) 
				maxlen = Math.max(maxlen, stack.peek() + curlen);
			else
